 ...
 * `--canvas <slugOrId>` plus a hidden, deprecated `--canvas-id` alias kept for
 ...
 */
export const withCanvasOption = (cmd: Command, description = 'Canvas slug or ID'): Command =>
  cmd
    .addOption(markExampleRequired(new Option('--canvas <slugOrId>', description)))
    .addOption(new Option('--canvas-id <slugOrId>', 'Deprecated alias for --canvas').hideHelp());
...
export const resolveCanvasSlugOrId = (options: CanvasOptionFlags): string => {
  ...
  if (!value) {
    throw new CliUsageError("required option '--canvas <slugOrId>' not specified");
  }
  return value;
};
